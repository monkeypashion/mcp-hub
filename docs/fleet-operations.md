# Fleet operations — squads, seats, placements, transport, hooks

Moved out of CLAUDE.md on 2026-09-02 (bar 62: the file is loaded into every
lane's first turn, and prose that restates a mechanism does not need to be).
**Nothing here was rewritten** — these sections are verbatim, so the warnings
they carry still read exactly as they did. CLAUDE.md keeps a hard-rules
summary and points here for the mechanism.
