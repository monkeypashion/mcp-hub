...
class TestTag:
    def test_silence_means_session_verified(self):
        assert _grade_tag_str("session-verified") == ""
...
class TestReadTools:
    ...
    async def test_get_messages_is_silent_for_session_verified(self, server, db):
        await _call(server, "register", {"name": "bob", "project": "p"})
        await _call(server, "send", {"from_agent": "alice", "to": "bob",
                                     "message": "hi"})
        _set_grade(db, "alice", "session-verified")
        out = await _call(server, "get_messages", {"agent_name": "bob"})
        assert "alice" in out and "·asserted" not in out and "·ungraded" not in out
